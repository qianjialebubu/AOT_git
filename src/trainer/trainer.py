...
class Trainer():
    ...
    def save(self):
        if self.args.global_rank == 0:
            dir = os.path.join(self.args.save_dir,"PT")
            # print(f'\nsaving {self.iteration} model to {self.args.save_dir} ...')
            # dir = "D:\\file\\program_data\\deeplearning\\AOT-GAN-for-Inpainting-master\\experiments\\module"
            # pthG = "netG.pt"
            # pthD = "netD.pt"
            # pthO = "netO.pt"
            # print(f'G{str(self.iteration).zfill(7)}.pt')
            # torch.save(dir,pthG)
            # torch.save(dir,pthD)
            # torch.save(dir,pthO)
            # print(self.netG.module.state_dict())
            # print(os.path.join(self.args.save_dir()))
            # print('--------------------------------')
            # print(self.netG.state_dict())
            # print('--------------------------------')
            torch.save(self.netG.state_dict(),
                os.path.join(dir, f'G{str(self.iteration).zfill(7)}.pt'))
            torch.save(self.netD.state_dict(),
            # torch.save(self.netD.module.state_dict(),
                os.path.join(dir, f'D{str(self.iteration).zfill(7)}.pt'))
            torch.save(
                {'optimG': self.optimG.state_dict(), 'optimD': self.optimD.state_dict()},
                os.path.join(dir, f'O{str(self.iteration).zfill(7)}.pt'))

    # 训练模型
    def train(self):
        # 设定画布。dpi越大图越清晰，绘图时间越久
        # self.args.iterations = 100000
        ...
        for idx in pbar:
                ...
                for key, val in losses.items():
                    if ((self.iteration % 1000 == 0)):
                        if (key == "L1"):
                            ...
                            mm += 1
                            y1 = np.append(y1, val.item())
                        if (key == "Style"):
                            y2 = np.append(y2, val.item())
            ...
            if self.args.global_rank == 0 and (self.iteration % self.args.save_every) == 0: 
                self.save()
        # 绘图命令
        x = list(np.arange(1, mm + 1))

        plt.plot(x, y1, lw=1, ls='-', c='k', alpha=1)
        plt.plot(x, y2, lw=1, ls='-', c='b', alpha=1)
        plt.plot()
        # show出图形
        plt.show()
        # 保存图片
