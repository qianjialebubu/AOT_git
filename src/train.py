...
def main_worker(id, ngpus_per_node, args):
    ...
    if (not args.distributed) or args.global_rank == 0:
        print(f'using GPU {args.world_size}-{args.global_rank} for training')
        # print("Distributed_False")
        os.makedirs(args.save_dir, exist_ok=True)
        # 生成一个配置文件
        with open(os.path.join(args.save_dir, 'config.txt'), 'w+') as f:# 连接字符串构成文件名，打开文件进行写入
...
if __name__ == "__main__":
    ...
    # ngpus_per_node： 查看可用显卡的数目
    ngpus_per_node = torch.cuda.device_count()
    if ngpus_per_node > 10:
